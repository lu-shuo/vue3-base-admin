...
import { CustomOptions, UploaderEvents } from './typings';
...
const uploaderIns = computed(() => uploaderRef.value?.uploader);
const uploadBtnRef = ref(); // 选择文件按钮（页面内使用时使用）
const panelShow = ref(false); // 显示面板
const collapse = ref(false); // 收缩面板
const customStatus = ref(''); // 自定义状态

const trigger = (e: UploaderEvents) => {
	mittBus.emit(e);
	emits(e);
};
...
const initOptions = {
	...
	testChunks: true,
	// 服务器分片校验函数，秒传及断点续传基础
	checkChunkUploadedByResponse(chunk: any, message: string) {
		let skip = false;
...
const customizeOptions = (opts: CustomOptions) => {
	// 自定义上传url
	if (opts.target) {
...
// 计算md5
const computeMD5 = (file: any): Promise<string> => {
	// 文件状态设为"计算MD5"
	...
	// 计算MD5时隐藏”开始“按钮
	nextTick(() => {
		(document.querySelector(`.file-${file.id} .uploader-file-resume`) as HTMLElement).style.display = 'none';
	});
	// 开始计算MD5
		...
		generateMD5(file, {
			onProgress(currentChunk: number, chunks: number) {
				...
				nextTick(() => {
					...
					(document.querySelector(`.custom-status-${file.id}`) as HTMLElement).innerText = md5ProgressText;
				});
			},
			onSuccess(md5: string) {
...
function statusSet(id: number, status: keyof typeof statusMap) {
